...
var PyGoModuleMethodForwardCallBack = syscall.NewCallback(func(self uintptr, args uintptr) uintptr {

	pyArgs := AsPyTuple(args)
	pyArgsLen := pyArgs.Size()
	if pyArgsLen < 1 {
		return PyReturnNone().Instance()
	}
...
func CreateGoModule(name string, doc string) *PyGoModule {
	...
	methodCallDef := cpy.PyMethodDef{
		...
		Ml_meth:  PyGoModuleMethodForwardCallBack,
		Ml_flags: 1,
...
func NewModuleInitFuncCallBack(moduleName string, fn func() *PyGoModule) uintptr {
